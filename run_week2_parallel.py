...
def compute_vocab_features(df: pd.DataFrame, text_col: str = 'text', n_process: int = 2) -> pd.DataFrame:
    ...
    Steps (and why):
      1) Tokenize in parallel with spaCy using a lightweight pipeline.  
         → Fast + memory-friendly.
      2) Keep ONLY alphabetic tokens (tok.is_alpha).  
         → Drops URLs, emojis, numbers, punctuation so counts reflect real words.
      3) Compute lexical metrics: word_count, unique_word_count, TTR, avg_word_length.  
         → Basic breadth/complexity signals.
      4) Compute MTLD on the CLEANED word stream (joined tokens).  
         → Length-stable lexical diversity, unaffected by links/punct.
    """
    # 1) Load spaCy tokenizer-only pipeline to save memory
    nlp = spacy.load("en_core_web_sm", disable=["parser", "ner"])  # we only need tokenization

    # 2) Convert entire text column to list → efficient for nlp.pipe
    texts = df[text_col].tolist()

    # 3) Tokenize in parallel; batch_size balances speed vs. memory
    docs = list(nlp.pipe(texts, n_process=n_process, batch_size=500))

    # 4) Build a CLEAN list of lowercase alphabetic tokens (no URLs, emojis, punctuation, numbers)
    #    Using spaCy's token flags is Alpha (letters) → TRUE words
    df['tokens'] = [[tok.text.lower() for tok in doc if tok.is_alpha] for doc in docs]

    # 5) Basic lexical metrics computed from CLEAN tokens
    df['word_count'] = df['tokens'].map(len)
    df['unique_word_count'] = df['tokens'].map(lambda toks: len(set(toks)))
    # Type-Token Ratio (diversity measure)
    df['ttr'] = df.apply(
        lambda row: row['unique_word_count'] / row['word_count'] if row['word_count'] > 0 else 0,
        axis=1
    )
    # Average token length (characters)
    df['avg_word_length'] = df['tokens'].map(lambda toks: np.mean([len(w) for w in toks]) if toks else 0)

    # 6) MTLD is computationally expensive; parallelize across cores
    #    IMPORTANT: compute on CLEANED words so URLs/emoji/punct do not inflate diversity
    def mtld_metric_from_tokens(tok_list) -> float:
        if not tok_list:
            return 0.0
        try:
            # Join clean tokens back into a string for LexicalRichness
    ...
    # 7) (Optional) If memory is tight, you can drop 'tokens' here.  
    #    Keep it if later steps need word-level info beyond counts.
...
def compute_sentiment_features(df: pd.DataFrame, text_col: str = 'text', n_jobs: int = 2) -> pd.DataFrame:
    ...
    Steps:
      ...
      2) If a user column exists, compute mean/std of compound per user.  
         → Captures typical tone and emotional variability.
    """
    analyzer = SentimentIntensityAnalyzer()
    texts = df[text_col].tolist()

    # Parallel polarity scoring
    ...
    # Per-user volatility (if user_id/author/username exists)
...
def compute_style_features(df: pd.DataFrame, text_col: str = 'text') -> pd.DataFrame:
    ...
    Note: We intentionally compute these on RAW text so emoji/punct are preserved
          as style signals, separate from lexical diversity.
    """
    ...
    # Requires word_count computed in vocab step (clean words) → stable denominator
...
def compute_topic_features(
    ...
    Why this design?
      - Fitting clustering on all docs is memory-heavy.  
      - Transforming is lightweight: it projects new docs into the learned topic space.
    """
    # 1) Draw a random sample for fitting the cluster structure
    ...
    # Use GPU for BERTopic's internal embeddings if available
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    embedding_model = SentenceTransformer('all-MiniLM-L6-v2', device=device)

    tm = BERTopic(
        embedding_model=embedding_model, # forces GPU embeddings inside BERTopic
        umap_model=umap_model, 
        hdbscan_model=hdbscan_model, 
        low_memory=True, # chunk internal steps to loweer RAM/VRAM
        calculate_probabilities=False, # skip heavy topic-prob matrix (faster/lighter)
        verbose=False
    )

    # 3) Fit on the sample (learn topic space)
    _topics_sample, _ = tm.fit_transform(sample_texts)

    # 4) Assign topics to EVERY comment using the learned space
    all_texts = df[text_col].tolist()
    topics_all, _ = tm.transform(all_texts)  # lightweight assignment

    # 5) Save into df (vectorized assignment; no merges needed)
...
def compute_embedding_features(
    ...
    Dense semantic features (GPU-friendly + memory-safe):
      ...
      2) Encode texts in SMALL BATCHES (chunked loop) to avoid VRAM spikes
      3) Stack into a single matrix and reduce to 34 dimensions with PCA
    """
    model = SentenceTransformer(model_name, device='cuda')  # use GPU if available
    texts = df[text_col].tolist()

    # Encode in chunks: predictable memory usage and fewer OOMs
    all_embs = []
    ...
    # PCA → compact, decorrelated features (cheaper for downstream models)
...
def aggregate_user_mtld(df: pd.DataFrame) -> pd.DataFrame:
    """
    Aggregate comment-level MTLD values to compute per-user mean/std.
    Useful when training user-level models or adding stability to noisy per-comment signals.
    """
    user_col = next((c for c in ['user_id', 'author', 'username'] if c in df.columns), None)
    if user_col:
...
def main():
    ...
    """
    # 1) Ensure required NLP resources are present
    ensure_nltk_resources()
    ensure_spacy_model("en_core_web_sm")

    # 2) Load the cleaned dataset from Week 1
    root = Path(__file__).parent
    df = pd.read_parquet(root / 'data' / 'pandora_cleaned.parquet')

    # 3) Feature engineering steps (order matters: style uses word_count from vocab)
    df = compute_vocab_features(df, n_process=4)
    df = compute_sentiment_features(df, n_jobs=4)
    df = compute_style_features(df)
    df = compute_topic_features(df, sample_size=10000)  # fit on sample, label all
    df = compute_embedding_features(df, batch_size=32)

    # 4) User-level aggregation
    df = aggregate_user_mtld(df)

    # 5) Save the fully-featured DataFrame back to Parquet
